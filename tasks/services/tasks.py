...
from tasks.models import ExamIncorrectWordQuestion, ExamOptionsQuestion, IncorrectWordQuestionBase, IncorrectWordQuestionBlank, OptionsQuestionBase, OptionsQuestionBlank, Task, UserExam



def exam_incorrect_word_question_create_from_blank(*, exam: UserExam, blank: IncorrectWordQuestionBlank, commit: bool = True) -> ExamIncorrectWordQuestion:
    """Создает объект вопроса с неправильной буквой для испытания."""        
    new_instance_data = {
        x.name: getattr(blank, x.name)
        for x in IncorrectWordQuestionBase._meta.get_fields()
    }
...
def exam_options_question_create_from_blank(*, exam: UserExam ,blank: OptionsQuestionBlank, commit: bool = True) -> ExamOptionsQuestion:
    ...
    donor_field_pairs = [
        (x, y)
        for x, y in donor_field_pairs
        if getattr(blank, x)
    ]

    for (acceptor_field1, acceptor_field2), (donor_field1, donor_field2) in zip(acceptor_field_pairs, donor_field_pairs):
...
@transaction.atomic
def exam_create_by_task(*, task: Task, user: User) -> tuple[UserExam, ExamOptionsQuestion | ExamIncorrectWordQuestion | None]:
    ...
    for question in questions[: task.max_questions_count]:
        exam_question = (
            ...
            if isinstance(question, OptionsQuestionBlank) else
            exam_incorrect_word_question_create_from_blank(exam=exam, blank=question, commit=False)
        )
