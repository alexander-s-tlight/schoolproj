...
EXAM_QUESTION_MODEL_BY_TYPE = SimpleLazyObject(lambda: {
    x.QUESTION_TYPE: x
    for x in  (ExamIncorrectWordQuestion, ExamOptionsQuestion)
})
...
class Task(models.Model):
    ...
    max_questions_count = models.PositiveIntegerField(verbose_name='максимальное количество вопросов в испытании', default=1)
...
class IncorrectWordQuestionBlank(IncorrectWordQuestionBase):
    ...
    def clean(self) -> None:
        ...
        for iter_num, (char1, char2) in enumerate(zip(self.correct_word, self.incorrect_word), start=1):
            if char1 != char2:
...
class OptionsQuestionBlank(OptionsQuestionBase):
    ...
    def clean(self) -> None:
        """Очищает и проверяет данные модели."""
        if not (
            self.option1_is_true or self.option2_is_true or self.option3_is_true 
        ):
...
class ExamIncorrectWordQuestion(ExamQuestionFinishedMixin, IncorrectWordQuestionBase):
    ...
    created_at = models.DateTimeField(verbose_name='дата добавления', auto_now_add=True)
    selected_letter_index = models.IntegerField(verbose_name='выбранный номер неправильной буквы', null=True, blank=True)
